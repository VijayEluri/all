...
public class SmsPopupDbAdapter
{
	...
	private static final String QUICKMESSAGES_UPDATE_ALL_ORDERING = "update quickmessages set ordering=ordering+"
			+ QUICKMESSAGES_ORDERING_DEFAULT;
	private static final String QUICKMESSAGES_GET_MAX_ORDERING = "select max(" + KEY_ORDER + ") from "
			+ QUICKMESSAGES_DB_TABLE;
	private static final String QUICKMESSAGES_GET_MIN_ORDERING = "select min(" + KEY_ORDER + ") from "
			+ QUICKMESSAGES_DB_TABLE;
	 ...
	 */
	private static final String CONTACTS_DB_CREATE = "create table " + CONTACTS_DB_TABLE + " (" + KEY_CONTACT_ID
			+ " integer primary key, " + KEY_CONTACT_NAME + " text default 'Unknown', " + KEY_ENABLED
			+ " integer default 1, " + KEY_POPUP_ENABLED
			+ " integer default 1, "
			+ KEY_RINGTONE
			+ " text default '"
			+
			// + Settings.System.DEFAULT_NOTIFICATION_URI.toString() + "', " +
			"content://settings/system/notification_sound" + "', " + KEY_VIBRATE_ENABLED + " integer default 1, "
			+ KEY_VIBRATE_PATTERN + " text default '0,1200', " + KEY_VIBRATE_PATTERN_CUSTOM + " text null, "
			+ KEY_LED_ENABLED + " integer default 1, " + KEY_LED_PATTERN + " text default '1000,1000', "
			+ KEY_LED_PATTERN_CUSTOM + " text null, " + KEY_LED_COLOR + " text default 'Yellow', "
			+ KEY_LED_COLOR_CUSTOM + " text null, " + KEY_SUMMARY + " text default 'Default notifications' " + ");";

	/**
	 * Table creation sql statement
	 */
	private static final String QUICKMESSAGES_DB_CREATE = "create table " + QUICKMESSAGES_DB_TABLE + " (" + KEY_ROWID
			+ " integer primary key autoincrement, " + KEY_QUICKMESSAGE + " text, " + KEY_ORDER + " integer default "
			+ QUICKMESSAGES_ORDERING_DEFAULT + ");";
	...
	public long createContact(long contactId)
	{
		...
		if (c == null) {
			...
			String contactName = SmsPopupUtils.getPersonName(context, String.valueOf(contactId), null);
			if (contactName == null)
				return 0;
	...
	public boolean deleteContact(long contactId, boolean showToast)
	{
		if (mDb.delete(CONTACTS_DB_TABLE, KEY_CONTACT_ID + "=" + String.valueOf(contactId), null) > 0) {
			...
			if (showToast) {
				Toast.makeText(context, R.string.contact_customization_removed_toast, Toast.LENGTH_SHORT).show();
			}
			return true;
		}
		return false;
	...
	{
		boolean found = false;
		Cursor mCursor = mDb.query(true, CONTACTS_DB_TABLE, new String[] { KEY_CONTACT_ID, KEY_CONTACT_NAME,
				KEY_RINGTONE }, KEY_CONTACT_ID + "=" + contactId, null, null, null, null, null);
		if (mCursor != null) {
	...
	{
		boolean found = false;
		Cursor mCursor = mDb.query(true, CONTACTS_DB_TABLE, null, // Return all
																	// columns
				KEY_CONTACT_ID + "=" + contactId, null, null, null, null, null);
		if (mCursor != null) {
			found = mCursor.moveToFirst();
		}
		if (!found) {
		...
		if (Log.DEBUG)
			Log.v("fetchContactSettings - found contact in db, " + contactId);
		return mCursor;
	}
	...
	public Cursor fetchAllContacts()
	{
		return mDb.query(CONTACTS_DB_TABLE,
				new String[] { KEY_CONTACT_ID, KEY_CONTACT_NAME, KEY_RINGTONE, KEY_SUMMARY }, null, null, null, null,
				KEY_CONTACT_NAME);
	}
	...
	/*
	 * This updates a summary column in the database that contains a brief
	 * summary of the notification settings for the contact.
	 */
	...
	public Cursor fetchQuickMessage(long id) throws SQLException
	{
		...
		Cursor mCursor = mDb.query(true, QUICKMESSAGES_DB_TABLE,
				new String[] { KEY_ROWID, KEY_QUICKMESSAGE, KEY_ORDER }, KEY_ROWID + "=" + id, null, null, null, null,
				null);
		if (mCursor != null) {
			found = mCursor.moveToFirst();
		}
	...
	{
		return mDb.query(QUICKMESSAGES_DB_TABLE, new String[] { KEY_ROWID, KEY_QUICKMESSAGE, KEY_ORDER }, null, null,
				null, null, QUICKMESSAGES_SORT_BY);
	}
	...
	public boolean reorderQuickMessage(long id)
	{
		...
		if (minOrderNum == -1 || minOrderNum == 0)
			return false;
	...
	/*
	 * Gets the minimum ordering value in the quick message table (or -1 if
	 * there's an error)
	 */
	...
	/*
	 * Gets the maximum ordering value in the quick message table (or -1 if
	 * there's an error)
	 */
